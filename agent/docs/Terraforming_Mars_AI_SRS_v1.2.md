...
## 2.6 Assumptions and Dependencies
  ...
  - The Engine can be run headless and driven programmatically at high throughput (thousands of games), which the RL phases depend on. This throughput assumption - and in particular the cost of snapshotting/cloning state for search - is **not yet validated** and must be confirmed by the Milestone 1 performance spike (see NFR-2 and the Implementation Plan). If clone cost proves prohibitive, search depth and RL scale are rescoped accordingly; the assumption is load-bearing and treated as a risk until measured.
...
**NFR-2** In embedded mode the decision core plus Engine shall sustain throughput sufficient for self-play at scale (target: on the order of thousands of complete games per day on a single multi-core workstation for the heuristic/search agent; RL throughput as budgeted in the plan). The cost of snapshotting/restoring (cloning) Engine state is the binding constraint on both search depth and self-play scale and shall be **measured before Milestones 4 and 6 are committed** (see the Implementation Plan Milestone 1 spike). A hard target - state-clone cost <= X, with X derived from the simulations-per-move the search actually needs - shall be established from that measurement; if it cannot be met, search depth and RL scale shall be rescoped rather than the target silently missed. (Note: “thousands of games/day” suffices for heuristic tuning but is far below AlphaZero-scale self-play; the RL ambition of Milestone 6 must be budgeted against measured throughput, not assumed.)
