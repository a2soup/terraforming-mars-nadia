...
**Project:** Terraforming Mars AI **Document:** Software Requirements Specification (SRS) **Version:** 1.9 (Draft) **Date:** 11 August 2026 **Prepared for:** Austin Campbell **Target scope:** Base game + Corporate Era + Prelude; 2-4 players (2p primary; 3-4p competence per AC-5) **Primary interface:** terraforming-mars open-source engine (self-hosted “Heroku app”)

**Revision history**

  - **v1.9 (11 Aug 2026):** **Milestone 2 is complete.** Bullet 5's regression suite is built and adjudicated (agent/docs/Regression_Suite.md) against nine criteria pre-committed before any code: all nine met, over 13 agent-independent reference positions, 33 pinned reference games and 11 pre-registered negative controls run against two corpora. **One correction to this document, made in place in the §2.6 annotation.** Through v1.8 that annotation routed the eight Engine-vs-print divergences to "the Milestone 2 regression seed set" as their only downstream consumer. A seed set alone cannot discharge them: a hash over a whole game reports the same event whether a card's value changed or the enumerator reordered two options, and the requirement those divergences carry is that a *silent* value regression be caught. The mechanism is now a **direct assertion per divergence** plus a pinned corpus that proves the line is reached - both, not either - and this was measured rather than argued (a mutation to Hackers' `bespokePlay` moved **zero** pinned entries on a 10-game corpus and was caught only by the direct assertion). No requirement text, acceptance criterion, milestone or scope boundary changed; the annotation is a correction of mechanism, not of intent. Recorded alongside it: `agent/docs/data/card_play_coverage.json` carries one wrong zero (Sell Patents, an instrument artefact, not a reachability fact) which the audit's other rows do not share.
  - **v1.8 (10 Aug 2026):** **FR-DATA-1 is discharged** by the reduced Milestone 2 bullet 4 (agent/docs/Corporation_Prior.md) - 17 of 17 corporations reconciled, nothing unmatched in either direction, two project-independent arithmetic identities holding exactly. The requirement is annotated in place. **Two corrections this document needed and the ingestion forced**, which is why this is a version bump rather than an additive annotation: **WAP is not a win rate** (§1.5 said it was; it is a mean Elo-performance residual on ~[-2, +2], not a probability, and FR-DATA-3's "prefer WAP" was therefore an instruction with a mis-stated object), and **the corpus is 3-player while the primary setting is 2-player**, so the surviving prior is biased by a player-count difference no version of this document had flagged. Both are recorded rather than resolved: the prior stays because FR-DATA-2 keeps it weak and FR-DATA-4 lets the harness overrule it. No requirement text changed beyond FR-DATA-1's annotation, and no acceptance criterion, milestone or scope boundary changed.
  - **v1.7 (10 Aug 2026):** **AC-8 is withdrawn and the expert dataset's role is cut back to a single weak opening prior.** Before starting Milestone 2 bullet 4 the cost of the distributional calibration was re-examined against what it could ever return, and it did not survive the comparison: AC-8 was already a smell test that could not fail the project (no threshold, no consequence), yet reaching it required reconciling the full ~200-card BGA card set against the engine, computing a formal distributional report, and generating a dedicated 3-player calibration corpus — the most expensive work in Milestone 2 in service of the one criterion whose own text says it is "never a skill threshold, a target to imitate, or a constraint." **Cut:** AC-8 itself; the full card-set reconciliation; the formal distributional report over score / TR / generation counts and per-card win rates; and the 3-player calibration game generation. **Kept:** the per-corporation win rates as a weak opening-book prior, which needs 17 in-scope corporations reconciled by hand rather than the whole card pool. FR-DATA-1 is narrowed to that ingestion; FR-DATA-2 loses its card/corporation profile sanity-check clause. **FR-DATA-2 through FR-DATA-5 are otherwise unchanged and still binding** — the guardrails constrain the prior that remains, and the project's exposure to the data is now smaller than the guardrails were written for, not larger. The primary bar (AC-1 + AC-4 + AC-6) and every other criterion are untouched; supporting evidence is now AC-3 and AC-5. No functional requirement outside FR-DATA-1/2 changed, and no milestone other than Milestone 2 bullet 4 changed scope.
  - **v1.6 (27 Jul 2026):** The Milestone-1 **card-coverage audit** (Implementation Plan bullet 7, agent/docs/Card_Coverage_Audit.md) is complete, and the **§2.6 rules-ground-truth assumption is annotated in place** with its result. The assumption is **substantially validated but not unconditionally true**: every printed in-scope card/prelude/corporation is present and reachable-as-classified, the whole 73-card logic-bearing surface was read against the printed cards, and 99%+ is exercised by both the Engine suite and 1,500 games of play — but the audit found **eight Engine-vs-print divergences** (five scoring/legality-relevant), and did not read the 204 declarative cards line-by-line or audit milestones/awards. The premise is not weakened operationally (the Agent plays the Engine's game correctly by CON-1); the exceptions are carried into FR-DATA-1's reconciliation (§3.5) and the M3 evaluator. No requirement text changed; the §2.6 annotation is additive.
  - **v1.5 (24 Jul 2026):** **AC-1 and NFR-4 are met and evidenced for the Milestone-1 random-legal agent** (agent/docs/AC1_Legality_Run.md): 1,500 consecutive embedded games in one process - 1,000 at 2p plus 250 each at 3p/4p - with 1,500 completions, zero crashes, zero unrecovered illegal moves and **zero Agent-attributable illegal-move rejections across 444,680 submissions**. The run fixes the ambiguity AC-1 carried since v1.0 by defining an illegal move as *a move submitted to the Engine and rejected by it*, and by counting the three distinct populations of rejection separately rather than conflating them with the FR-9 fallback's firing rate. Both criteria are annotated in place with the measurement and with the standing caveat that they are agent-specific and must be re-run for each promoted version - the pre-fix run showed a defect that had been hidden behind the FR-9 fallback since the enumerator was written. No requirement, scope or acceptance-criteria changes.
...
## 1.4 References
  ...
  - **Expert data:** github.com/RuneDK93/terraforming-mars-dataset - aggregate statistics from ~1,556 base + Corporate Era 3-player games (BGA season 18) and ~1,616 base + Prelude 3-player games (BGA season 19), all played by the top 25 rated players. Only the per-corporation win-rate table is consumed (v1.7; see Section 3.5).

  - Companion document: “Terraforming Mars AI - Implementation Plan,” v1.9.
...
**The RuneDK93 expert dataset.** Aggregate win-rate statistics computed from ~1,500-1,600 games per configuration played by the top 25 rated Board Game Arena players, including a base + Prelude 3-player corpus that matches this project’s scope. It reports score, Terraform Rating, generation counts, and per-card and per-corporation win rates, each alongside a **WAP** column. **As of v1.7 the project consumes exactly one part of it: the per-corporation win-rate table, as a weak opening-book prior** (Section 3.5, FR-DATA-1).

> **WAP, corrected at v1.8 (10 Aug 2026).** Through v1.7 this document described WAP as “a skill-adjusted win rate that partially controls for player strength.” It is skill-adjusted; it is **not** a win rate. Upstream it is the mean over a corporation’s games of (actual finishing score `[2,1,0]` by position) minus (the sum over opponents of the Elo win probability) - a **mean Elo-performance residual in pairwise wins per game**, on roughly [-2, +2] and centred near zero. It cannot be read as a probability, mixed with one, or rescaled to [0, 1]. Found by actually ingesting the table (agent/docs/Corporation_Prior.md); FR-DATA-3’s instruction to prefer this column stands and now has a defined object. The wider distributional comparison it would support was withdrawn along with AC-8 - it required reconciling the full BGA card set against the engine to produce a check that, by its own definition, could never fail the project. Critically, the repository distributes aggregate statistics, not move-by-move game logs; obtaining move-level data for supervised learning would require a separate scraping pipeline subject to the platform’s Terms of Service. The cautions in Section 3.5 govern how this data may and may not be used.

## 1.6 Document Overview

Section 1.5 summarises the prior art and available data resources and the limits on their use. Section 2 describes the Agent in context and the constraints it operates under. Section 3 defines the external interfaces, including the exact observation and action model exposed by the Engine and, in Section 3.5, the expert-data interface and the guardrails on it. Section 4 lists functional requirements. Section 5 lists non-functional (quality) requirements. Section 6 defines the acceptance criteria, including a concrete, externally grounded definition of “highly skilled” (an expert-human benchmark and expert review; the dataset is not an acceptance criterion). Appendices provide the game-scope detail and a glossary of the decision types.
...
## 2.6 Assumptions and Dependencies
  ...
  - The chosen Engine commit correctly implements the base game, Corporate Era, and Prelude rules; its card implementations are treated as the rules ground truth.
    ...
    > **Audited at Milestone 1 (27 Jul 2026)** — agent/docs/Card_Coverage_Audit.md. This assumption is **substantially validated but not unconditionally true**. Every printed in-scope card, prelude and corporation is present (277/277) and reachable-as-classified; the whole logic-bearing surface (73 cards) was read against the printed cards; 99%+ is exercised by both the Engine suite and 1,500 games of play. The audit found **eight cards whose Engine behaviour diverges from the printed card** (five scoring/legality-relevant), so "correctly implements the rules" now holds **with a catalogued list of exceptions** rather than as a blanket guarantee. This does **not** weaken the ground-truth premise operationally: the Agent plays the Engine's game correctly by CON-1, and no divergence makes it play illegally or mis-score in its own world. It means the M3 evaluator fits to Engine value, not printed value. (Through v1.6 it also obliged the FR-DATA-1 card-set reconciliation to treat those cards as known Engine-specific rules when comparing against the printed-rules BGA dataset. **That reconciliation was withdrawn in v1.7 along with AC-8**, and all eight divergences are project cards rather than corporations, so the surviving corporation-only ingestion does not touch them. The divergences remain live for the M3 evaluator and for the Milestone 2 regression suite, which is now their only downstream consumer. **Discharged at Milestone 2 bullet 5, 11 Aug 2026** - agent/docs/Regression_Suite.md - and by a different mechanism than "the regression seed set" named through v1.8: each divergence has an **L1 reference position asserting the Engine's number directly**, and the pinned corpus separately proves the line is reached. Both are required. A pinned seed alone cannot distinguish a card's value changing from an enumerator reordering, which is precisely the silent regression these rows exist to catch, and the negative controls measured it: a mutation to one of the two untested effects moved **zero** pinned entries on a 10-game corpus.) Two further limits are recorded: the 204 purely-declarative cards were **not** read line-by-line (a wrong `behavior` value would not have been caught by the reading, only indirectly by test/play coverage), and milestones/awards were **not** audited. Full catalogue and the known-limitations register are in the audit document.
  ...
  - The expert dataset (Section 1.5) is treated as a source of one weak prior, not as a specification of correct play and no longer as a benchmarking resource. It is aggregate statistics, not move logs; it originates from the Board Game Arena implementation rather than the primary engine, so the 17 in-scope corporations are reconciled by name against the engine before their win rates are used. **No card-set-wide reconciliation is assumed or required** - the comparison that would have needed one was withdrawn in v1.7. Any use of move-level data depends on a separate, ToS-compliant data pipeline that is not assumed to exist at project start.
...
The expert dataset of Section 1.5 is a supporting resource, not part of the game interface. This section defines how the system may consume it and - equally important - the limits that prevent it from distorting the Agent’s development. The governing principle is that the data informs the Agent; it never defines correct play.

> **Narrowed at v1.7 (10 Aug 2026).** Through v1.6 this section also governed a like-for-like distributional comparison between the Agent’s own games and the expert corpus, feeding the AC-8 calibration check. **AC-8, that comparison, and the card-set reconciliation it required are withdrawn.** What survives is FR-DATA-1's corporation-only ingestion below. FR-DATA-2 through FR-DATA-5 are unchanged in force: they now constrain a smaller surface than they were written for, which is the safe direction.

**FR-DATA-1** The system shall be able to ingest the dataset’s per-corporation win-rate table for the base + Prelude 3-player corpus and, after reconciling the 17 in-scope corporation names against the primary engine’s identifiers, expose it as a weak opening-book prior for corporation selection (Implementation Plan, Milestone 2 bullet 4; consumed at Milestone 3). Corporations present in one source but not the other shall be flagged and left without a prior rather than silently coerced. The system is **not** required to ingest per-card win rates, score, Terraform Rating or generation-count distributions, nor to compute any of them from the Agent’s own games.

> **Discharged at Milestone 2, bullet 4 (10 Aug 2026)** — agent/docs/Corporation_Prior.md. `agent/src/prior/corporationPrior.ts` over a committed artifact (`docs/data/corporation_prior.json`) built from the upstream table vendored verbatim at `docs/data/runedk93_prelude_corps.txt`, which hashes to its upstream blob SHA. **Reconciliation is 17 of 17 with nothing unmatched in either direction**, so the flag-don’t-coerce branch never fires on the real table and is driven by a synthetic row in the spec instead. Two arithmetic identities hold exactly and are independent of this project: participations sum to 3 × 1,616 and wins sum to 1,616, the latter also establishing that the published “Win Rate” is a first-place rate. There is no CLI - the spec re-derives the artifact from the vendored source, so a hand edit to the JSON fails there.
>
> **Two things the ingestion established that this document had wrong or unstated.** (i) **WAP is not a win rate.** §1.5 below is corrected: it is a mean Elo-performance residual in pairwise wins per game, on ~[-2, +2], computed upstream as the mean of `[2,1,0]-by-position` minus the Elo-expected score. It is genuinely the skill-adjusted column - FR-DATA-3 is right to prefer it - but it is not a probability and must not be rescaled into one. (ii) **The corpus is 3-player and the primary setting is 2-player**, so the prior is biased by a player-count difference nothing in this document had flagged; corporation strength is not player-count invariant. Acceptable only because FR-DATA-2 keeps the prior weak and FR-DATA-4 lets the harness overrule it.
>
> **Most of the table is not separated from chance.** Eight of the seventeen have a 95% interval excluding the 1/3 baseline; the middle nine span -5.0 to +3.9 pp with intervals that all contain it. The loader ships the interval and a `separatedFromChance` flag so a consumer does not rank noise. Separately, the skill adjustment reorders the table where it matters most: Spearman 0.92 overall, but CrediCor is 1st by raw rate and 6th by WAP.

**FR-DATA-2** The system shall use expert statistics only as weak priors - for example, to seed an opening-selection prior. It shall not set evaluation-function weights directly equal to observed win rates.
...
> **Discharged at Milestone 2, bullet 1 (28 Jul 2026)** — agent/docs/Match_Runner.md. The runner (`agent/src/match/`, behind `npm run match`) plays a named, **versioned** lineup at 2-4 players under a pairing-group design that fixes one Engine seed per group and plays every seat permutation, so a win rate is not confounded with seat order; it records outcomes, placements (not merely win/loss - AC-5 is stated in placement terms), and move histories at three tiers, the fullest of which is verified by re-deriving each game from its seeds. Adjudicated against eight criteria (R1-R8) pre-committed in their own commit before any measurement code existed: **seven met**, with parallel throughput (R7) recorded as *untested* because the measurement host was swapping and no valid number could be taken - it is one command on an idle host. Two further limits are recorded there: the first-seat advantage the pairing corrects was **not** demonstrated at 1,000 games of random-legal play (the pairing is correct; its necessity is unproven, and is to be re-measured at M3 with an agent that can exploit tempo), and the ranking's full-tie path is covered by construction only. The AC-1 legality accounting of FR-13's sibling requirement NFR-4 is **absorbed** into this runner as an opt-in mode, verified equivalent to the Milestone-1 runner counter-for-counter.
...
> **Discharged at Milestone 2, bullet 3 (31 Jul 2026)** — agent/docs/Rating_Pipeline.md. `agent/src/rating/` behind `npm run rate` computes win rate, placement rate and average VP margin with intervals, one-sided threshold tests for the shape every acceptance criterion takes, a sample-size/power calculator, a promotion gate, and a committed ladder. **The unit of analysis is the pairing group, never the game**: games in a group share an Engine seed, and the measured design effects (1.033 / 1.252 / 1.293 at 2p/3p/4p on the win rate, 1.043 / 1.516 / 1.184 on the margin) make an uncorrected 3p margin interval 23% too narrow, in the stratum where AC-5 lives. Point estimates are unaffected; only intervals change.
>
> **"Elo/TrueSkill" is discharged by the Bradley-Terry / Plackett-Luce family, and TrueSkill is deliberately not implemented.** The slash is a family, not two requirements. What ships is maximum-a-posteriori Bradley-Terry at 2p and Plackett-Luce over 3-4p placements - one latent strength, a weak Gaussian prior, presented on the Elo scale with `random-legal@1` anchored at 0 - with intervals from a seeded cluster bootstrap and never from the Hessian, and separation decided structurally so a pair the data cannot bound above reports an explicitly unbounded end rather than a large finite number. TrueSkill's value is online updating over a large, continuously changing ladder; this project has a handful of frozen versions and complete match records, and re-fitting takes milliseconds. Its σ is the width of an approximate-message-passing posterior, and treating that as a confidence interval is precisely the unvalidated claim this bullet's coverage study exists to prevent - whereas a bootstrap interval around a MAP fit can be, and was, coverage-tested. If a later milestone needs online updating over a large evolving pool, TrueSkill goes behind the same interface.
>
> **The rating is a summary and never a gate.** Not one criterion in Section 6 is stated as an Elo; AC-2, AC-3, AC-5 and AC-7 are each a rate with a threshold, so the promotion gate is a one-sided cluster-corrected test on the head-to-head win rate. With two identities at 2p the Elo *is* that win rate, monotonically transformed.
>
> **Soundness was measured, not asserted, and two of the nine pre-committed criteria are not met.** The calibration study (160 coverage cells against known ground truth, 60 size cells, 5 power cells) found and fixed three real defects, and its verdicts are recorded honestly: coverage (P2) is not met, and the analytic anchor shows the pre-committed [94.0%, 96.0%] band is unattainable by *any* interval in four of its sixteen anchor cells - exact Wilson coverage at p = 0.99, n = 100 is 92.06% by enumeration - with the pipeline's own contribution within ±1.7 pp and mean coverage 0.9503 where arithmetic allows. Power (P3) misses one of five rows by 0.44 pp of tolerance below 150 games, documented and printed by the CLI. Full adjudication in the deliverable.
...
**NFR-4** In embedded mode the Agent shall produce **zero** Agent-attributable illegal-move rejections, and its unhandled-error rate shall be low enough that the probability of any failure across a 1,000-game run is under 5% (i.e., per-game failure probability <= ~5x10^-5) - consistent with AC-1’s zero-failure requirement over 1,000 games. **Verified for the Milestone-1 random-legal agent (24 Jul 2026): 0 Agent-attributable rejections in 444,680 submissions across 1,500 games, and 0 failures against a tolerance of ~5x10^-5 per game (agent/docs/AC1_Legality_Run.md). Reaching zero required a fix the run itself found - 59 rejected `initialCards` submissions caused by a budget constraint enforced on the composite rather than on the sub-input the Agent answers - so this requirement is one that measurement, not inspection, establishes.** In live-play mode the Agent shall recover from transient network/server errors and shall never leave a game in a stalled state attributable to the Agent.
...
Because “highly skilled” is the crux of this project, it is pinned to concrete, measurable thresholds. Strength is always measured on the target scope (base + Corporate Era + Prelude), with two-player play as the primary configuration, over statistically adequate sample sizes. The skill bar itself is defined by external, skill-sensitive play - a quantitative expert-human benchmark (AC-4) and a qualitative expert review (AC-6) - not by resemblance to any dataset. **No acceptance criterion refers to the expert dataset.** A distributional match to a dataset is neither necessary nor sufficient for skill: a self-play sample can resemble expert statistics without the Agent being expert-strength, which is why the bar rests on external opposition rather than on distributional resemblance - and, from v1.7, why the distributional check that once stood here as AC-8 was withdrawn rather than kept as a criterion no result could ever fail.

| ID   | Criterion                                            | Threshold                                                                                                                                                                                                                                                                                                                                                                    |
| ---- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC-1 | Legality and completion - **MET for the Milestone-1 random-legal agent (24 Jul 2026)** | Plays >= 1,000 consecutive embedded games with zero illegal moves and zero unhandled errors. **Measured: 1,500 consecutive games (1,000 at 2p, 250 each at 3p/4p) in a single process - 1,500 completed, 0 crashes, 0 unrecovered illegal moves, 0 Agent-attributable illegal-move rejections across 444,680 submissions (agent/docs/AC1_Legality_Run.md). "Illegal move" is defined there as a move submitted to the Engine and rejected by it. This criterion is agent-specific and must be re-run for each promoted agent version.**                                                                                                                                                                                                                                                                                |
...
| AC-5 | Multiplayer competence                               | In 3-4 player games, finishes first at a rate significantly above the 1/N chance baseline (e.g., >= 2x expected placement). **Tested since Milestone 2 bullet 3. Two cautions the pipeline enforces: the null is `seats held / players`, not `1/N`, whenever one identity occupies more than one seat in the lineup; and 3-4p is where the clustering correction matters most (design effect 1.25-1.29 on the win rate, up to 1.52 on the margin), so an uncorrected 3p test over-rejects at 7.6% against a nominal 5%.** |
...
| AC-7 | Monotonic improvement (guardrail)                    | Each promoted version beats the previous version with statistical significance in the match harness. **The test exists since Milestone 2 bullet 3 (31 Jul 2026): `npm run rate -- gate` runs a one-sided cluster-corrected test on the head-to-head win rate against a pre-registered sample size and a reserved seed range (agent/docs/Rating_Pipeline.md). Two costs are now measured rather than assumed. (i) Sample size: 1,000 games resolves a 4.0 pp edge and nothing finer, and certifying a 53% improvement takes 1,767 games - at Milestone 4's search budget roughly 4.7 days single-core, so from M4 onward the choice of N is a multi-day commitment to be budgeted from `npm run rate -- power`, not inherited from what was run last time. (ii) Discipline: testing after every 50 pairing groups turns a 4.9% test into 16.5%, and selecting the best of 8 variants then reporting the games they were selected on inflates the win rate by 2.2 pp. Seed blocks are allocated and a gate's range is reserved before it runs.** |
| ~~AC-8~~ | ~~Distributional calibration (supporting sanity check)~~ | **WITHDRAWN at v1.7 (10 Aug 2026).** Formerly: after BGA-vs-engine reconciliation, the Agent’s own game statistics are not wildly inconsistent with the top-25 expert distributions of Section 1.5. Withdrawn because its own text made it unfalsifiable as a criterion - "a smell test only, never a skill threshold, a target to imitate, or a constraint" - while satisfying it required the most expensive work in Milestone 2 (a full card-set reconciliation, a formal distributional report, and a dedicated 3-player calibration corpus). **The ID is retired, not reused.** |

**Primary bar:** AC-1, AC-4, and AC-6 together constitute the definition of success: the Agent must play legally to completion (AC-1) and demonstrate expert-level strength both quantitatively against strong external opposition (AC-4) and qualitatively under expert review (AC-6). AC-2 and AC-7 are improvement guardrails. AC-3 and AC-5 are supporting evidence.

**Note on the dataset.** The expert distributions are a reference, not a target, and since v1.7 the project does not compare against them at all: matching them is neither necessary nor sufficient for skill, so a criterion built on resemblance bought nothing that AC-4 and AC-6 do not buy better. One use survives - a weak opening-book prior over corporation selection (FR-DATA-1) - and it remains bound by FR-DATA-2/3/4. The Agent is expected and encouraged to exceed expert tendencies and to discover play the dataset does not exhibit, and shall never be scored against, penalised for, or constrained toward conformity with them.
...
**Player counts.** 2 to 4 players. Two-player games are the primary training and evaluation setting (simplest to train and evaluate head-to-head, and the setting in which the AC-4 human benchmark is run). Three- and four-player competence is required (AC-5). The top-25 three-player expert dataset of Section 1.5 is **not** benchmarked against in any configuration - the calibration that once used it was withdrawn at v1.7 - so no 3-player games need be generated for its sake. The solo variant is not a target but is a useful smoke test.

# Appendix B. Traceability Note

Each functional requirement in Section 4 and interface requirement in Section 3 maps to one or more acceptance criteria in Section 6 and to a milestone in the companion Implementation Plan. FR-OBS/FR-ACT/FR-INT establish the interface the Agent must master (Milestones 1-2); FR-4 through FR-8 establish decision quality (Milestones 3-5); FR-DATA-1 through FR-DATA-5 govern the one surviving use of the expert data - the corporation opening prior - and the guardrails on it (Milestones 2-3 and 6); since v1.7 they map to **no** acceptance criterion, which is the point of the withdrawal. FR-13 through FR-15 and NFR-3 establish the evaluation and training machinery that verifies AC-1 through AC-7.

Terraforming Mars AI - SRS v1.8 | Page
