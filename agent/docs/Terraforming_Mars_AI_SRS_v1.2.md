...
## 1.4 References
  ...
  - Companion document: “Terraforming Mars AI - Implementation Plan,” v1.8.
...
**The RuneDK93 expert dataset.** Aggregate win-rate statistics computed from ~1,500-1,600 games per configuration played by the top 25 rated Board Game Arena players, including a base + Prelude 3-player corpus that matches this project’s scope. It reports score, Terraform Rating, generation counts, and per-card and per-corporation win rates. **As of v1.7 the project consumes exactly one part of it: the per-corporation win-rate table, as a weak opening-book prior** (Section 3.5, FR-DATA-1). The wider distributional comparison it would support was withdrawn along with AC-8 - it required reconciling the full BGA card set against the engine to produce a check that, by its own definition, could never fail the project. Critically, the repository distributes aggregate statistics, not move-by-move game logs; obtaining move-level data for supervised learning would require a separate scraping pipeline subject to the platform’s Terms of Service. The cautions in Section 3.5 govern how this data may and may not be used.
...
Terraforming Mars AI - SRS v1.7 | Page
